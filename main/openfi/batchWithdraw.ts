...
/**
 * Perform OpenFi withdraw operations for a single wallet in parallel
 */
async function withdrawForWallet(
  ...
  try {
    console.log(
      `[${wallet.name}] Starting OpenFi withdraw from address: ${wallet.address}`
    );
    ...
    // Randomly select an asset to withdraw
    ...
    try {
      ...
      console.log(`[${wallet.name}] Successfully withdrew from OpenFi`);
      return {
        success: true,
        wallet: wallet.name,
      ...
      console.error(`[${wallet.name}] OpenFi withdraw failed: ${error}`);
      return {
        success: false,
        wallet: wallet.name,
        address: wallet.address,
...
/**
 * Main function to run parallel OpenFi withdraw operations for all wallets
 */
async function main() {
  try {
    ...
    console.log(
      `[BATCH MODE] Will run withdraw ${concurrentTasks} times as specified in config`
    );
    console.log(`Running all wallets in parallel (batch mode)`);

    // Execute withdraw operations for all wallets in parallel
    const startTime = Date.now();
    let totalSuccessful = 0;
    let totalOperations = 0;

    // Run withdraw operations concurrentTasks times (91 times as per config)
    for (let i = 0; i < concurrentTasks; i++) {
      console.log(`\n[BATCH ROUND] Starting round ${i + 1}/${concurrentTasks}`);

      const results = await executeAllWalletsInParallel(
        walletManager,
        async (wallet, index) => withdrawForWallet(wallet, index, 50) // Use 50% for amount in percent
      );
      ...
      console.log(
        `[BATCH ROUND ${i + 1}] ${successful}/${
          results.length
        } OpenFi withdraw operations were successful`
      );
    ...
    // Print summary
    console.log(
      `\n[BATCH SUMMARY] ${totalSuccessful}/${totalOperations} OpenFi withdraw operations were successful across ${concurrentTasks} rounds`
    );
    console.log(
    ...
    console.error(
      "[BATCH ERROR] Error running parallel OpenFi withdraw operations:",
      error
    );
  }
}
