...
const assetAddress = [
  pharosTokenAddress[0].address,
  pharosTokenAddress[1].address,
  mockUSD,
];

export async function advisor({ baseDir, wallet, provider }: Advisor) {
  dotenv.config({ path: path.join(baseDir, ".env") });
  const { PROXY_URL = "", AUTOSTAKING_TOKEN = "" } = process.env!;
  const advisorUrl =
    "https://asia-east2-auto-staking.cloudfunctions.net/auto_staking_pharos_v7/investment/financial-portfolio-recommendation";
  ...
  for (const asset of assetAddress) {
    ...
    const assetsUsd =
      parseInt(assetBalance.toString(16), 16) / 10 ** Number(assetDecimals);
    const data = {
      address: asset,
      assets: `${uint256Hex}`,
      assetsUsd,
